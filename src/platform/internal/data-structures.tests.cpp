...
void RunHeapArrayTests()
{
	...
	*firstItemInRemainingSpace = 3;
	...
	for (int i = 0; i < 6; i += 1)
	{
		sixInts[i] = originalMemValue;
	}
	HeapArray<int> deleteArray = HeapArray<int>(fourInts);

	deleteArray.append(1);
	deleteArray.append(2);
	deleteArray.append(3);
	deleteArray.append(4);
	assert(deleteArray.length == 4);

	deleteArray.deleteContent(1, 2);

	assert(deleteArray.length == 2);
	assert(sixInts[0] == originalMemValue);
	assert(sixInts[1] == 1);
	assert(sixInts[2] == 4);
	assert(sixInts[3] == 3);
	...
	deleteArray.deleteContent(0, 0);

	assert(deleteArray.length == 3);
	assert(sixInts[0] == originalMemValue);
	assert(sixInts[1] == 2);
	assert(sixInts[2] == 3);
	assert(sixInts[3] == 4);
	...
	deleteArray.deleteContent(0, 3);

	assert(deleteArray.length == 0);
	assert(sixInts[0] == originalMemValue);
	assert(sixInts[1] == 1);
	assert(sixInts[2] == 2);
	assert(sixInts[3] == 3);
